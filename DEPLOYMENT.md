...
- **“No open HTTP ports detected” / Port scan timeout:** Render expects your app to listen on `0.0.0.0` and the port in the `PORT` env (usually 10000). The start script uses this. If you still see this: (1) Confirm **Start Command** is exactly `bash scripts/start.sh`. (2) Set **Health Check Path** to `/api/health/` so Render checks HTTP instead of only port scan. (3) In **Advanced**, ensure no custom **Port** override conflicts with your app. (4) Redeploy; if it persists, try Start Command `gunicorn core.wsgi --bind 0.0.0.0:$PORT` and run migrations in the Build Command (e.g. add `&& python manage.py migrate --noinput`).
